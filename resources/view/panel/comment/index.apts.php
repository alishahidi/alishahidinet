...
<title>پنل کاربری کامنت ها</title>
@endsection
                ...
                <tbody>
                    ...
                    foreach ($notApprovedComments as $comment) { ?>
                        <tr>
                            <td><?= $i ?></td>
                            <td>
                                <a href="<?= route('panel.comment.edit', [$comment->id]) ?>" class="ms-3">
                                    <ion-icon class="text-success" name="pencil-outline"></ion-icon>
                                </a>
                                <form class="d-inline" action="<?= route('panel.comment.delete', [$comment->id]) ?>" method="post" onclick="return confirm('آیا مایلید حذف شود؟')">
                                    @token
                                    @method('DELETE')
                                    <button>
