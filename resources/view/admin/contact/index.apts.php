                ...
                <tbody>
                    ...
                    foreach ($contacts as $contact) { ?>
                        <tr>
                            <td><?= $i ?></td>
                            <td>
                                <a href="<?= route('admin.contact.view', [$contact->id]) ?>" class="ms-3">
                                    <ion-icon class="text-primary" name="send-outline"></ion-icon>
                                </a>
                            </td>
                            <td><?= e($contact->name) ?></td>
                            <td><?= e($contact->email) ?></td>
                            <td><?= e($contact->subject) ?></td>
                            <td><?= e(limitDotPrint($contact->text, 30)) ?></td>
                            <td><?= e($contact->support_key) ?></td>
                            <td><?= jdate($contact->created_at)->format('%A, %d %B %y') ?></td>
                            <td><?= $contact->updated_at ? jdate($contact->updated_at)->format('%A, %d %B %y') : '--' ?></td>
                        </tr>
                    <?php $i++;
                    } ?>
                </tbody>
