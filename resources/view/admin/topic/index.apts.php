    ...
    <div class="card-body">
        <div class="card-table">
            <div class="border-bottom mb-4 pb-3">
                <a href="<?= route('admin.topic.create') ?>" class="custom-btn custom-btn-primary text-center rounded-0 w-100">ساخت تاپیک جدید</a>
            </div>
            <table id="table" class="display nowrap">
                ...
                foreach ($topics as $topic) { ?>
                        <tr>
                            <td><?= $i ?></td>
                            <td>
                                <a href="<?= route('admin.topic.edit', [$topic->id]) ?>" class="ms-3">
                                    <ion-icon class="text-success" name="pencil-outline"></ion-icon>
                                </a>
                                <form class="d-inline" action="<?= route('admin.topic.delete', [$topic->id]) ?>" method="post" onclick="return confirm('آیا مایلید حذف شود؟')">
                                    @token
                                    @method('DELETE')
                                    <button>
                            ...
                            <td><?= e($topic->name) ?></td>
                            <td><?= jdate($topic->created_at)->format('%A, %d %B %y') ?></td>
                            <td><?= $topic->updated_at ? jdate($topic->updated_at)->format('%A, %d %B %y') : '--' ?></td>
                        </tr>
                    <?php $i++;
                } ?>
                </tbody>
