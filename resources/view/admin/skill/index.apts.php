...
<title>پنل ادمین مهارت ها</title>
@endsection
    ...
    <div class="card-body">
        <div class="card-table">
            <div class="border-bottom mb-4 pb-3">
                <a href="<?= route('admin.skill.create') ?>" class="custom-btn custom-btn-primary text-center rounded-0 w-100">ساخت مهارت جدید</a>
            </div>
            <table id="table" class="display nowrap">
                ...
                foreach ($skills as $skill) { ?>
                        <tr>
                            <td><?= $i ?></td>
                            <td>
                                <a href="<?= route('admin.skill.edit', [$skill->id]) ?>" class="ms-3">
                                    <ion-icon class="text-success" name="pencil-outline"></ion-icon>
                                </a>
                                <form class="d-inline" action="<?= route('admin.skill.delete', [$skill->id]) ?>" method="post" onclick="return confirm('آیا مایلید حذف شود؟')">
                                    @token
                                    @method('DELETE')
                                    <button>
                            ...
                            <td><?= e($skill->name) ?></td>
                            <td><?= e($skill->percent) ?></td>
                            <td><?= jdate($skill->created_at)->format('%A, %d %B %y') ?></td>
                            <td><?= $skill->updated_at ? jdate($skill->updated_at)->format('%A, %d %B %y') : '--' ?></td>
                        </tr>
                    <?php $i++;
                } ?>
                </tbody>
